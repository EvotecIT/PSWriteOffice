param(
    ...
    [string] $SourceTable = 'dbo.PSWriteOfficeCsvSource',
    [string] $DestinationTable = 'dbo.PSWriteOfficeCsvRoundTrip',
    [string] $Path,
    [int] $RowCount = 100,
...
if (-not $Path) {
    ...
    $Path = Join-Path $outputDirectory 'Csv-DbaClientXRoundTrip.csv'
}
