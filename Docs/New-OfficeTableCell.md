...
### __AllParameterSets
```powershell
New-OfficeTableCell [[-Text] <string>] [-ColumnSpan <int>] [-RowSpan <int>] [<CommonParameters>]
```
...
### EXAMPLE 1
```powershell
PS> $row = @(New-OfficeTableCell -Text 'Identity systems' -ColumnSpan 3)
```

The returned cell can be passed to WordTable or PdfTable inside explicit row arrays.
